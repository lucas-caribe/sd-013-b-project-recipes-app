...
import { fetchRandomMeal, fetchRandomDrink } from '../Redux/Actions';
...
function ExploreBy() {
  ...
  const handleCLick = ({ target: { name } }) => {
    if (url === 'comidas') history.push(`/explorar/comidas/${name}`);
    if (url === 'bebidas') history.push(`/explorar/bebidas/${name}`);
  };
